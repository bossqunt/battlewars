...
<div class="mb-4">
  <form method="GET" class="flex gap-2">
    <input type="text" name="search" placeholder="Search items..." class="p-2 border rounded" value="<?= htmlspecialchars($_GET['search'] ?? '') ?>">
  ...
  <table class="min-w-full bg-white border border-gray-200 rounded shadow text-xs">
    <thead>
      <tr class="bg-gray-100">
        ...
        <th class="p-2 border-b">Attack</th>
        <th class="p-2 border-b">Defense</th>
        <th class="p-2 border-b">Speed</th>
        <th class="p-2 border-b">Crit Multi</th>
        <th class="p-2 border-b">Crit Chance</th>
        <th class="p-2 border-b">Life Steal</th> 
    ...
    <tbody>
      <?php foreach ($listings as $listing): ?>
        <tr class="border-b hover:bg-gray-50">
          ...
          <td class="p-2"><?= (int)($listing['attack'] ?? 0) ?></td>
          <td class="p-2"><?= (int)($listing['defense'] ?? 0) ?></td>
          <td class="p-2"><?= (int)($listing['speed'] ?? 0) ?></td>
          <td class="p-2"><?= (int)($listing['crit_multi'] ?? 0) ?></td>
          <td class="p-2"><?= (int)($listing['crit_chance'] ?? 0) ?></td>
          <td class="p-2"><?= (int)($listing['life_steal'] ?? 0) ?></td>
            ...
            <?php if (($listing['player_id'] ?? null) == ($playerId ?? null)): ?>
              ...
              <?php if (!empty($listing['offer'])): ?>
                <form method="POST" action="api/marketAcceptOffer.php" class="inline market-action-form">
                  ...
                  <button type="submit" class="bg-green-500 text-white px-2 py-1 rounded text-xs">Accept Offer (<?= (int)($listing['offer_amount'] ?? $listing['offer']) ?>g)</button>
                </form>
              <?php endif; ?>
            <?php else: ?>
              ...
              <form method="POST" action="api/marketOffer.php" class="inline w-full mt-1 market-action-form">
                <input type="hidden" name="listing_id" value="<?= $listing['id'] ?>">
                <input type="number" name="offer_amount" min="1" placeholder="Offer" class="w-16 p-1 border rounded text-xs">
